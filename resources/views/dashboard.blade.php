                ...
                <div class="bg-[#1e2030] border-2 border-[#984063] rounded-[22px] p-8 text-white">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <div class="text-right">
                            <span class="text-gray-400 text-xs uppercase block">House ID</span>
                            <span class="text-2xl font-black text-[#FE9677] tracking-widest">{{ $colocation->join_code }}</span>
                        </div>
                    </div>
                                ...
                                <div class="flex justify-between items-center bg-[#0f1015] p-4 rounded-xl border border-[#41436A]">
                                    ...
                                    <div class="flex items-center gap-4">
                                        ...
                                        @if($colocation->pivot->group_role === 'owner' && $member->id !== Auth::id())
                                            <form method="POST" action="{{ route('colocations.remove_member', $member->id) }}" onsubmit="return confirm('Are you sure you want to kick {{ $member->name }}?');">
                                                @csrf
                                                @method('DELETE')
                    ...
                    @if($colocation->pivot->group_role === 'owner')
                        ...
                        <div class="border-t border-[#41436A] pt-6 mt-6 flex justify-between items-center">
                            ...
                            <form method="POST" action="{{ route('colocations.deactivate') }}" onsubmit="return confirm('Are you sure you want to delete this house? This cannot be undone.');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" 
